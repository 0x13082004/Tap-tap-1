...
let _chainOkCache = false;

// Prevent overlapping interactive wallet requests (common cause of "already pending"/"unable to sign").
let _connectPromise = null;
let _chainSwitchPromise = null;
...
async function getProvider() {
  ...
  if (window.ethereum) {
    _providerCache = window.ethereum;
...
async function warmupWallet() {
  try {
    ...
    // Non-interactive calls only (no popups on load).
    const accounts = await provider.request({ method: "eth_accounts" }).catch(() => []);
    _accountCache = accounts?.[0] || null;

    const chainId = await provider.request({ method: "eth_chainId" }).catch(() => null);
    _chainOkCache = chainId === BASE_MAINNET_CHAIN_ID;
  } catch {
    // ignore
  }
}

async function ensureAccount(provider) {
  if (_accountCache) return _accountCache;

  // If a connect request is already in-flight, await it instead of starting another.
  if (_connectPromise) {
    const accounts = await _connectPromise;
    _accountCache = accounts?.[0] || null;
    return _accountCache;
  }

  _connectPromise = provider.request({ method: "eth_requestAccounts" });
  try {
    const accounts = await _connectPromise;
    _accountCache = accounts?.[0] || null;
    return _accountCache;
...
async function ensureBaseMainnet(provider) {
  if (_chainOkCache) return BASE_MAINNET_CHAIN_ID;
  ...
  // Only one chain switch prompt at a time.
  if (!_chainSwitchPromise) {
    _chainSwitchPromise = provider.request({
...
async function walletSendCallsTap({ counter }) {
  ...
  const from = await ensureAccount(provider);
  ...
  // One tap = one onchain call. We DO NOT wait for mining; we only wait for the wallet to accept & submit.
...
async function processTapQueue() {
  ...
  try {
    while (tapQueue > 0) {
      // One tap = one wallet request (no batching), but we do NOT wait for mining.
      const currentCounter = Number(localStorage.getItem("tapCounter") || "0") || 0;
      const nextCounter = currentCounter + 1;

      // Reserve one queued tap for this send.
      tapQueue -= 1;
      ...
      try {
        ...
        // Mark as submitted immediately (no receipt wait).
        ...
        // If the wallet says a request is already pending, put the tap back and retry shortly.
        if (low.includes("already pending") || low.includes("request") && low.includes("pending")) {
          tapQueue += 1;
          setTapHint(`Waiting for wallet… (${tapQueue} queued)`);
          await new Promise((r) => setTimeout(r, 400));
          continue;
        }

        if (low.includes("user rejected") || low.includes("rejected") || low.includes("cancel")) {
          toast("Cancelled");
        } else if (low.includes("insufficient") || low.includes("not enough")) {
          toast("Insufficient funds");
        } else if (low.includes("sign")) {
          toast("Unable to sign transaction. Try again.");
        } else {
...
window.addEventListener("load", async () => {
  ...
  await safeReady();
  // Pre-warm provider/chain/account to make the first tap popup appear faster.
  void warmupWallet();
});
