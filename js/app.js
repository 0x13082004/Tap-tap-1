...
async function walletSendCallsTap({ startCounter, count }) {
  const provider = await getProvider();

  // Reuse cached account when possible to reduce popup delay.
  let from = _accountCache;
  if (!from) {
  ...
  // Build a single EIP-5792 request containing N calls.
  // This avoids the "unable to sign" error that happens when a second request is sent
  // before the wallet has fully finished handling the previous one.
  const calls = [];
  ...
  const req = {
    ...
    // Many hosts execute `wallet_sendCalls` sequentially (not atomically).
    // Setting this to `false` improves compatibility.
...
async function processTapQueue() {
  ...
  try {
    while (tapQueue > 0) {
      // Take a snapshot of the queued taps and send them in ONE wallet request.
      // This lets the user keep tapping without waiting for onchain confirmation.
      const batch = tapQueue;
      tapQueue = 0;

      const currentCounter = Number(localStorage.getItem("tapCounter") || "0") || 0;
      const startCounter = currentCounter + 1;

      setTapHint(`Confirm in wallet… (${batch} queued)`);

      try {
        await walletSendCallsTap({ startCounter, count: batch });

        const lastCounter = currentCounter + batch;
        localStorage.setItem("tapCounter", String(lastCounter));

        // Only after a successful wallet flow do we add Pata.
        pata += batch;
        store.setPata(pata);
        updateHud();
      } catch (e) {
