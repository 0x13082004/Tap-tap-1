...
function getBuilderCode() {
  try {
    const v = localStorage.getItem("builder_code");
    if (v && typeof v === "string" && v.trim() && !v.includes("TODO_REPLACE")) return v.trim();
  } catch {}
  return BUILDER_CODE;
}
...
function render() {
  ...
  if (state.mode === "offchain" || state.mode === "onchain") {
    // unlimited mode: no energy limits
  } else {
    regenEnergy();
  }
...
async function getDataSuffix() {
  // REQUIRED (as per your spec): keep the exact
async function ensureEnoughGas(provider, from) {
  // Any onchain tx on Base requires ETH for gas, even if value=0 and data=0x.
  try {
    const balHex = await provider.request({ method: "eth_getBalance", params: [from, "latest"] });
    const bal = BigInt(balHex || "0x0");
    if (bal === 0n) {
      throw new Error("No Base ETH for gas. Add a small amount of ETH on Base to pay fees.");
    }
  } catch (e) {
    // If wallet blocks balance reads, do not hard-fail; let wallet decide.
    if (e && /No Base ETH for gas/i.test(e.message || "")) throw e;
  }
}
 import line in js/erc8021_import.js, loaded lazily.
  try {
    const mod = await import("https://tapgorila.vercel.app/js/erc8021_import.js");
    return mod.toDataSuffix(getBuilderCode());
  } catch {
    return null;
  }
}
...
async function walletSendCalls({ to, data }) {
  ...
  if (!effectiveBuilder || effectiveBuilder.includes("TODO_REPLACE")) {
    throw new Error("Builder code missing. Set a builder code in js/app.js or store it in localStorage key: builder_code");
  }
...
async function onTap(ev) {
  ...
  // ONCHAIN: unlimited, each tap triggers a tx to the EARN_CONTRACT
  if (onTap._pending) return;
  onTap._pending = true;

  try {
    // pre-wallet animation (wallet blocks screen immediately)
    ...
    if (/user rejected|rejected|denied/i.test(msg)) {
      toast("Cancelled");
    } else if (/No Base ETH for gas/i.test(msg)) {
      toast("Gas er jonno Base ETH dorkar. USDC diye gas hoy na.", 3600);
    } else {
      toast(msg, 3600);
    }
  } finally {
    document.getElementById("tapArea")?.classList.remove("charging");
    onTap._pending = false;
  }
}
...
function bindUI() {
  ...
  tapEl?.addEventListener("pointerdown", onTap, { passive: true });
  tapEl?.addEventListener("touchstart", onTap, { passive: true });
  ...
  // builder code input (lets you set builder_code without redeploy)
  const bc = document.getElementById("builderCode");
  if (bc) {
    try { bc.value = localStorage.getItem("builder_code") || (BUILDER_CODE.includes("TODO_REPLACE") ? "" : BUILDER_CODE); } catch {}
    bc.addEventListener("input", () => {
      try { localStorage.setItem("builder_code", (bc.value || "").trim()); } catch {}
    });
  }
