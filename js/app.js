...
const TIP_RECIPIENT = TAP_CONTRACT;
...
async function walletSendCallsTap({ counter }) {
  ...
  // Preflight: avoid confusing "insufficient funds" by checking Base ETH.
  const bal = await getEthBalanceWei(provider, from);
  if (bal <= 0n) {
    throw new Error("No Base ETH in this wallet for gas");
  }
