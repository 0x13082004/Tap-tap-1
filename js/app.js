...
async function detectEarnCalldata(provider, from) {
  // Try common no-arg selectors. Fall back to empty calldata if no selector works.
  const candidates = [
    "0x",          // raw call (fallback/receive)
    "0x4e71d92d",  // claim()
    "0xfd221031",  // tap()
    "0xd389800f",  // earn()
    "0x1249c58b",  // mint()
  ...
  for (const data of candidates) {
    try {
      await provider.request({
        method: "eth_call",
        params: [{ from, to: EARN_CONTRACT, value: "0x0", data }, "latest"],
      });
      return data;
    } catch (e) {
      const msg = String(e?.message || e || "");
      if (/execution reverted|revert|invalid opcode/i.test(msg)) continue;
      // If wallet/provider refuses eth_call, just stop trying and use "0x"
      return "0x";
    }
  }
  return "0x";
...
async function walletSendCalls({ to, data }) {
  ...
  const dataSuffixValue = await getDataSuffix();
  if (!dataSuffixValue) throw new Error("Failed to load dataSuffix attribution library.");

  // Capability handling: wallets MUST reject unsupported capabilities unless marked optional.
  ...
  const params = {
    ...
    capabilities: { dataSuffix: supportsDataSuffix ? { value: dataSuffixValue } : { value: dataSuffixValue, optional: true } },
  };
