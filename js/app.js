...
// ---- EIP-5792 helpers ----
// Some wallets (e.g. Farcaster Wallet, Coinbase Wallet, etc.) support EIP-5792 `wallet_sendCalls`.
// Capabilities (like ERC-8021 `dataSuffix`) MUST be sent as objects ({ value, optional }).
// If a wallet does not support a capability and it's not marked optional, it MUST reject the request (EIP-5792).

function makeBatchId() {
  // EIP-5792 `id` is an arbitrary string for tracking; keep it unique per request.
  try {
    return (crypto?.randomUUID?.() || `batch_${Date.now()}_${Math.random().toString(16).slice(2)}`);
  } catch {
    return `batch_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  }
}

async function getCapabilitiesSafe(provider, address) {
  try {
    return await provider.request({ method: "wallet_getCapabilities", params: [address] });
  } catch {
    return null;
  }
}

function buildSendCallsCapabilities({ allowAttribution = true } = {}) {
  // Always mark attribution optional so the app still works in wallets that don't support dataSuffix.
  if (!allowAttribution) return undefined;
  return {
    dataSuffix: {
      value: dataSuffix,
      optional: true,
...
function appendDataSuffixToCalldata(calldata) {
  // For non-sendCalls fallbacks (eth_sendTransaction), we can still append the suffix ourselves.
  if (!dataSuffix || typeof calldata !== "string") return calldata;
  if (!calldata.startsWith("0x")) return calldata;
  return calldata + dataSuffix.slice(2);
}

function isSendCallsUnsupportedError(e) {
  const msg = String(e?.message || e || "").toLowerCase();
  const code = e?.code;
  // -32601 Method not found, 4200 Unsupported method, etc.
  return code === -32601 || code === 4200 || msg.includes("wallet_sendcalls") || msg.includes("method not found");
}
...
async function walletSendCallsTap({ counter }) {
  ...
  let calldata = abiEncodeLogAction(actionId, data);

  // Try EIP-5792 first (best UX + supports Base Account capabilities).
  const req = {
    version: "2.0.0",
    id: makeBatchId(),
    from,
    ...
    capabilities: buildSendCallsCapabilities({ allowAttribution: true }),
  };

  try {
    return await provider.request({ method: "wallet_sendCalls", params: [req] });
  } catch (e) {
    // If wallet_sendCalls isn't supported (or is blocked), fall back to eth_sendTransaction.
    if (!isSendCallsUnsupportedError(e)) throw e;

    const tx = {
      from,
      to: TAP_CONTRACT,
      value: "0x0",
      data: appendDataSuffixToCalldata(calldata),
    };

    // This fallback may require the user to have Base ETH for gas.
...
async function processTapQueue() {
  ...
  try {
    while (tapQueue > 0) {
      ...
      try {
        await walletSendCallsTap({ counter: nextCounter });
        localStorage.setItem("tapCounter", String(nextCounter));

        // Only after a successful wallet flow do we add 1 Pata.
        pata += 1;
        store.setPata(pata);
        updateHud();
      } catch (e) {
...
async function walletSendCallsUsdc({ usdString, recipient }) {
  ...
  let data = encodeErc20Transfer(recipient, units);
  ...
  try {
    ...
    const tx = {
      ...
      to: USDC_CONTRACT,
      value: "0x0",
      data: appendDataSuffixToCalldata(data),
    };
    return await provider.request({ method: "eth_sendTransaction", params: [tx] });
  }
}
