...
function setBuilderCode(v) {
  try {
    localStorage.setItem("builder_code", String(v || "").trim());
  } catch {}
}
...
function openSheet() {
  ...
  // preload builder code field
  const builderInput = document.getElementById("builderCode");
  if (builderInput && !builderInput.value) {
    const v = getBuilderCode();
    if (v && !v.includes("TODO_REPLACE")) builderInput.value = v;
  }
}
...
async function walletSendCalls({ to, data }) {
  ...
  if (!effectiveBuilder || effectiveBuilder.includes("TODO_REPLACE")) {
    throw new Error("Builder code missing. Open Tip tab and paste builder code.");
  }
...
async function sendTipUSDC(usd) {
  ...
  // store builder code if user typed it
  const builderInput = document.getElementById("builderCode");
  if (builderInput && builderInput.value && builderInput.value.trim()) {
    setBuilderCode(builderInput.value.trim());
  }
...
function bindUI() {
  ...
  // Save builder code on blur
  document.getElementById("builderCode")?.addEventListener("blur", (e) => {
    const v = String(e?.target?.value || "").trim();
    if (v) setBuilderCode(v);
  });
}
