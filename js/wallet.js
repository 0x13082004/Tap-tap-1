...
async function ensureBaseChain(provider) {
  const chainId = await provider.request({ method: "eth_chainId" });
      ...
      // Preflight: confirm the selected account actually has ETH on the current chain.
      // This helps catch the common case where the in-app wallet account is different from the one you funded.
      const balHex = await provider.request({ method: "eth_getBalance", params: [from, "latest"] });
      if (!balHex || balHex === "0x0") {
        toast("Selected wallet has 0 ETH on Base. Fund this address or switch wallet/account.");
        return false;
      }
...
async function sendSingleTransaction(provider, tx) {
  // Prefer a regular EIP-1193 transaction for maximum wallet compatibility.
  // Some wallets implement wallet_sendCalls via a Smart Account that may have a different gas balance,
  // which can surface as a misleading "insufficient funds" error even when the user's EOA has ETH.
  try {
    return await provider.request({ method: "eth_sendTransaction", params: [tx] });
  } catch (e) {
    // Fallback to EIP-5792 if the wallet doesn't support eth_sendTransaction.
    const msg = String(e && (e.message || e));
    if (msg.toLowerCase().includes("method not found") || msg.toLowerCase().includes("does not exist")) {
      const from = tx.from;
      const chainId = await provider.request({ method: "eth_chainId" });
      const suffix = safeDataSuffix();
      const caps = buildCapabilities(suffix, true);
      return await walletSendCalls(provider, from, chainId, [{
        to: tx.to,
        value: tx.value || "0x0",
        data: tx.data || "0x"
      }], caps);
...
window.walletBridge = {
  ...
  async sendOnchainTap() {
    ...
    try {
      ...
      await sendSingleTransaction(provider, { from, to: ONCHAIN_CONTRACT, value: "0x0", data });
      return true;
    } catch (e) {
      // Common wallet generic message; show our own clearer hint
      toast("Onchain tap failed. If you see \"enough funds\" again, confirm the selected account has Base ETH and that you are on Base Mainnet.");
      return false;
    }
  },
